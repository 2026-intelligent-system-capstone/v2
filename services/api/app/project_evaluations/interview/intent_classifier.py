...
def is_decline_response(text: str, llm: LlmClient) -> bool:
        ...
        [
            {
                ...
                "content": (
                    ...
                    "예시: '없습니다', '없어요', '괜찮습니다', '다 했습니다' 등은 yes.\n"
                    "'네' 한 단어는 모호하므로 no로 처리합니다.\n"
                    "추가 설명을 시작하면 no입니다.\n"
    ...
    normalized = response.strip().lower()
    if not normalized:
        logger.warning("decline parse failed: empty response — defaulting to False (keep asking)")
        return False
    ...
    if "no" in normalized:
        return False
    logger.warning(
        "decline parse failed: response=%r — defaulting to False (keep asking)",
        response,
    )
    return False
